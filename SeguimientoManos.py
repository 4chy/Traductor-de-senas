...
class detectormanos():
    ...
    def __init__(self, mode=False, maxManos = 2, model_complexity=1, Confdeteccion =0.5, Confsegui = 0.5):
        ...
        #Creamos los objetos que detectaran las manos y los dibujaran
    ...
    def encontrarmanos(self, frame, dibujar = True):
        imgcolor = cv2.cvtColor(frame, cv2.COLOR_BGR2HGB)
    ...
    def dedosarriba(self):
        dedos = []
        if self.lista[self.tip[0][1] > self.listaself.tip[0]-1][1]:
            dedos.append(1)
        else:
            dedos.append(0)
